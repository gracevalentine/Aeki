...
exports.topUpWallet = (req, res) => {
  ...
  const checkCustomerSql = 'SELECT wallet FROM customer WHERE customer_id = ?';
  database.query(checkCustomerSql, [user_id], (err, results) => {
    if (err) {
    ...
    const updateSql = 'UPDATE customer SET wallet = ? WHERE customer_id = ?';
    database.query(updateSql, [newBalance, user_id], (err2, result2) => {
      if (err2) {
        return res.status(500).json({ message: 'Gagal melakukan top-up.', error: err2 });
      }

      res.status(200).json({ message: 'Top-up berhasil.', topup_sebanyak : amount, saldo_terbaru: newBalance });
    });
  });
};
