...
override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ApiClient.authService.login(loginRequest).enqueue(object : Callback<UserResponse> {
            override fun onResponse(call: Call<UserResponse>, response: Response<UserResponse>) {
                if (response.isSuccessful && response.body()?.user != null) {
                    ...
                    Toast.makeText(requireContext(), "Login gagal. Cek kembali username/password", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    btnRegister.setOnClickListener {
        ...
        Toast.makeText(requireContext(), "Arahkan ke halaman registrasi", Toast.LENGTH_SHORT).show()
    }
}
}